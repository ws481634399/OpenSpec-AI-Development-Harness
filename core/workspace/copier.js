// TemplateCopier：node:fs/promises cp + filter（README 改名、workspace/repositories 排除、implementation 条件、--force 保护受保护目录）
import { cp, mkdir, copyFile } from 'node:fs/promises';
import { join, relative } from 'node:path';

const PROTECTED_WORLDS = ['standards', 'product', 'delivery', 'skills', 'prompts'];
 ...
 * - --force 时不触碰受保护目录（standards/product/delivery/skills/prompts），仅刷新 .sdd 静态文件
 *
...
function makeFilter(config, templateRoot) {
  return (src) => {
    ...
    if (config.force) {
      for (const w of PROTECTED_WORLDS) {
 ...
 *
 * - 静态文件直接复制（standards/ product/ delivery/ skills/ prompts/ .sdd/README.md .sdd/context-rules.yaml .sdd/version.yaml）
 * - 模板根 README.md 改名为 README-OpenSpec.md（绝不覆盖用户 README.md）
 * - implementation/ 仅当 config.shouldCreateImplementation 为真且非 force 时复制
 * - --force 时仅刷新 .sdd 静态文件 + README-OpenSpec.md，保护受保护目录与 implementation
