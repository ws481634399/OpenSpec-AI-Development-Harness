...
**Standards 晋升**（写入 `standards/`，子目录制，禁止写根下）：

- 新的技术规则或约定（编码规范、命名规则）→ `standards/engineering/coding-standard.md`
- 架构决策（模块划分、接口设计、技术选型）→ `standards/engineering/architecture-principles.md` 或 `standards/project/`
- 新的测试策略或工具配置 → `standards/engineering/testing-standard.md`
- 性能优化经验或教训 → `standards/engineering/` 对应主题文件
- 错误处理模式 → `standards/engineering/coding-standard.md`
- 安全实践 → `standards/engineering/security-guidelines.md`
...
- 新增或变更的业务能力 → `openspec feature` 命令更新 `feature-tree.yaml`
- Story 状态变更 → `openspec feature update <STORY-ID> --status delivered`

**Glossary 更新**（写入 `product/glossary/terms.md`）：

- 新术语或领域概念、术语定义修正
- 判断标准：**团队对同一概念的叫法和含义需要统一**

**禁止**：在 `product/` 根下新建任何 .md 文件（product 根只允许 feature-tree.yaml / INDEX.md / README.md；业务规则进 specs/，术语进 glossary/，能力变更进 feature-tree.yaml）
...
- 文件: standards/engineering/error-handling.md
- 操作: 新增
...
- 方式: openspec feature update STORY-3 --status delivered

### Glossary 更新

- 文件: product/glossary/terms.md
- 操作: 合并（已有文件追加条目）
- 内容: 「核销」= 券码从“已锁定”转为“已使用”的一次性动作
- 理由: 本 Change 引入新术语，需统一后续叫法
...
```markdown
- [x] 全部前序 Artifact 已读取
- [x] 知识分类完成（standards / spec 晋升 / feature-tree / glossary / no-update）
- [x] standards 更新已写入（如有）
- [x] Spec 晋升候选已按模板整理（如有，待人工评审）
- [x] Glossary 已更新（如有）
...
- `standards/engineering/<file>.md` 或 `standards/project/<file>.md` — 技术规则更新（如有）
- Spec 晋升候选（写入 convergence.md，人工评审后落 `product/specs/<feature-domain>.md`）
...
- 文件: standards/engineering/security-guidelines.md
- 操作: 更新（已有文件追加段落）
- 内容: 密码存储用 bcrypt，cost ≥ 10，不存明文
- 理由: 本 Change 实施密码安全存储，可作为后续认证相关 Change 参考

- 文件: standards/engineering/coding-standard.md
- 操作: 更新（已有文件追加段落）
