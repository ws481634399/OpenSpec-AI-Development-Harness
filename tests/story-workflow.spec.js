...
// 多 Story 中文名四级路径（与 fixture featurePath name 对齐）
const STORY_NAME = { [S1]: '用户注册', [S2]: '用户登录' };
const storyRel = (id) => `平台能力/用户中心/认证/${STORY_NAME[id]}`;
const storyDir = (changeDir, id) => join(changeDir, ...storyRel(id).split('/'));
...
async function makeMultiStoryChange({ status = 'designed' } = {}) {
  ...
  await writeChangeStories(changeDir, [
    { id: S1, title: '用户注册', inline: false, status: 'pending', path: `${storyRel(S1)}/` },
    { id: S2, title: '用户登录', inline: false, status: 'pending', path: `${storyRel(S2)}/` },
  ]);
...
test('多 Story: designed → run 进入 story-splitting，S1 spec 缺失 → WAITING_FOR_ARTIFACT(sdd-prd, S1) + Instruction 注入 Story 上下文', async () => {
  ...
  assert.ok(r.reason.includes(`${storyRel(S1)}/story-spec.md`), r.reason);
  // Change 已进入 story-splitting 结构态
  const meta = await readMetadata(changeDir);
  assert.equal(meta.status, 'story-splitting');
  // Story 状态未推进（等产物）
  const sm = await readStoryMetadata(storyDir(changeDir, S1));
...
test('多 Story: S1 story-spec 产出 → 机检 passed + story-spec 人审 required → WAITING_FOR_HUMAN', async () => {
  ...
  await writeFile(join(storyDir(changeDir, S1), 'story-spec.md'), storySpecMd(S1, ['S1']), 'utf8');
...
test('多 Story: S1 design+tasks 逐段推进 → S1 tasked → 循环切到 S2 spec 缺失（Story 状态留痕各自独立）', async () => {
  ...
  await writeFile(join(storyDir(changeDir, S1), 'story-design.md'), storyDesignMd(S1), 'utf8');
  ...
  const sm1 = await readStoryMetadata(storyDir(changeDir, S1));
  assert.equal(sm1.status, 'designed');

  // tasks 产出 → S1 tasked → 循环切 S2（spec 缺失）
  await writeFile(join(storyDir(changeDir, S1), 'tasks.md'), storyTasksMd(S1), 'utf8');
...
test('多 Story: 全部 Story tasked → §5.3 聚合推进 story-splitting→tasked → Change 级跳过 dev/test/review → converge 聚合门拦截', async () => {
  ...
  await writeFile(join(storyDir(changeDir, S2), 'story-spec.md'), storySpecMd(S2, ['S2']), 'utf8');
  ...
  await writeFile(join(storyDir(changeDir, S2), 'story-design.md'), storyDesignMd(S2), 'utf8');
  await runWorkflow(tmp, changeId, { harnessRoot }); // S2 tasks 缺失
  await writeFile(join(storyDir(changeDir, S2), 'tasks.md'), storyTasksMd(S2), 'utf8');
...
test('多 Story: 全部 Story completed → converge 聚合门放行（Change 不自动 completed，由 converge 人审驱动）', async () => {
  ...
  await patchStoryMetadata(storyDir(changeDir, S1), { status: 'completed' });
  await patchStoryMetadata(storyDir(changeDir, S2), { status: 'completed' });
...
test('Stale 传播: change-spec Gate 后变更 → 未完成 Story 的 story-spec 传播提示; story-design 变更 → tasks 传播; completed Story 不传播', async () => {
  ...
  await patchStoryMetadata(storyDir(changeDir, S1), { status: 'specified' });
  await patchStoryMetadata(storyDir(changeDir, S2), { status: 'completed' });
  ...
  await writeFile(join(storyDir(changeDir, S1), 'story-spec.md'), spec1, 'utf8');
  ...
  await writeFile(join(storyDir(changeDir, S1), 'story-design.md'), design1, 'utf8');
  ...
  await writeFile(join(storyDir(changeDir, S2), 'story-spec.md'), spec2, 'utf8');
  meta = await readMetadata(changeDir);
  ...
  await writeFile(join(storyDir(changeDir, S1), 'story-design.md'), design1 + '\n\n设计补丁', 'utf8');
  ...
  assert.ok(stale.some((s) => s.artifact === `${storyRel(S1)}/story-spec.md` && s.kind === 'stale-propagated' && s.from === 'change-spec.md'));
  // Story 级 hash 失配
  assert.ok(stale.some((s) => s.artifact === `${storyRel(S1)}/story-design.md` && s.kind === 'hash-mismatch' && s.layer === 'story'));
  // Story→tasks 传播
  assert.ok(stale.some((s) => s.artifact === `${storyRel(S1)}/tasks.md` && s.kind === 'stale-propagated' && s.from === 'story-design.md'));
...
test('Stale 传播: Story 级 rules-hash 不一致 → rules-mismatch（与 Change 级同构）', async () => {
  ...
  assert.ok(
    r.stale.some(
      (s) =>
        s.artifact === `${storyRel(S1)}/story-spec.md` &&
...
test('聚合边界: 任一 Story developing → 聚合推 Change.developing（tryAggregatedTransition 经 runWorkflow 验证）', async () => {
  ...
  await patchStoryMetadata(storyDir(changeDir, S2), { status: 'developing' });
