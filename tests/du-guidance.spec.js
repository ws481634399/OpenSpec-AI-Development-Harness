...
async function setupChange(tmp) {
  const { id, changeDir } = await runChangeCreate(
    tmp,
    { title: 'DU Guidance 测试', requirement: 'REQ-DUG', repositories: ['backend'] },
    harnessRoot
  );
...
test('writeWorkspaceDu 默认写入 guidance 段（sketch=true, pseudocode=false, trigger=[]）', async (t) => {
  ...
  const raw = await readFile(join(changeDir, 'FEAT-001', 'FEAT-001-01', 'FEAT-001-01-01', 'STORY-001-01-01-01', 'DU-BE-001', 'metadata.yaml'), 'utf8');
  const m = parse(raw);
...
test('writeWorkspaceDu 传 complexityTrigger → pseudocode 自动 true；非法 trigger 过滤', async () => {
  ...
  assert.deepEqual(m['implementation-guidance']['complexity-trigger'], ['business-flow']);
  await rmrf(tmp);
});
...
test('writeWorkspaceDu 显式 pseudocode=false 覆盖 trigger 联动', async () => {
  ...
  const m = parse(raw);
  assert.equal(m['implementation-guidance'].pseudocode, false);
  assert.deepEqual(m['implementation-guidance']['complexity-trigger'], ['algorithm']);
  await rmrf(tmp);
});

test('DU_COMPLEXITY_TRIGGERS 为四枚举', () => {
  assert.deepEqual([...DU_COMPLEXITY_TRIGGERS].sort(), ['algorithm', 'business-flow', 'orchestration', 'state-transition']);
});
...
test('Gate: du-guidance 完整 DU 小节 → passed', async () => {
  ...
  await writeFile(join(changeDir, 'tasks.md'), duSection(FULL_FIELDS), 'utf8');
...
test('Gate: du-guidance Sketch 为空/占位符 → failed', async () => {
  ...
  await writeFile(
    join(changeDir, 'tasks.md'),
    duSection(['- Implementation Sketch:', '- Pseudocode: N/A（配置变更）', '- Verification: CI 全绿']),
    'utf8'
  );
  let r = await runMachineGate(changeDir, gateOf('tasks.md', 'du-guidance'));
  assert.ok(r.issues.some((i) => i.includes('DU-BE-001 Implementation Sketch 为空')), r.issues.join('; '));
  // 占位符 Sketch
  await writeFile(
    join(changeDir, 'tasks.md'),
    duSection(['- Implementation Sketch: {{sketch}}', '- Pseudocode: N/A（配置变更）', '- Verification: CI 全绿']),
    'utf8'
  );
  r = await runMachineGate(changeDir, gateOf('tasks.md', 'du-guidance'));
  assert.ok(r.issues.some((i) => i.includes('Implementation Sketch 为空或含占位符')), r.issues.join('; '));
  await rmrf(tmp);
});
...
test('Gate: du-guidance pseudocode=true 但 N/A → failed；TBD → failed', async () => {
  ...
  await writeFile(
    join(changeDir, 'tasks.md'),
    duSection(['- Implementation Sketch: Controller → Service', '- Pseudocode: N/A（不想写）', '- Verification: 全绿']),
    'utf8'
  );
  let r = await runMachineGate(changeDir, gateOf('tasks.md', 'du-guidance'));
  assert.ok(r.issues.some((i) => i.includes('声明 pseudocode: true 但 Pseudocode 为空/占位符/N/A')), r.issues.join('; '));

  await writeFile(
    join(changeDir, 'tasks.md'),
    duSection(['- Implementation Sketch: Controller → Service', '- Pseudocode: TBD', '- Verification: 全绿']),
    'utf8'
  );
  r = await runMachineGate(changeDir, gateOf('tasks.md', 'du-guidance'));
  assert.ok(r.issues.some((i) => i.includes('声明 pseudocode: true 但 Pseudocode 为空/占位符/N/A')), r.issues.join('; '));
...
test('Gate: du-guidance pseudocode=false 但 Pseudocode 空 → failed；N/A 无理由 → failed', async () => {
  ...
  await writeFile(
    join(changeDir, 'tasks.md'),
    duSection(['- Implementation Sketch: Controller → Service', '- Pseudocode:', '- Verification: 全绿']),
    'utf8'
  );
  let r = await runMachineGate(changeDir, gateOf('tasks.md', 'du-guidance'));
  assert.ok(r.issues.some((i) => i.includes('Pseudocode 为空')), r.issues.join('; '));
  // N/A 无理由
  await writeFile(
    join(changeDir, 'tasks.md'),
    duSection(['- Implementation Sketch: Controller → Service', '- Pseudocode: N/A', '- Verification: 全绿']),
    'utf8'
  );
  r = await runMachineGate(changeDir, gateOf('tasks.md', 'du-guidance'));
  assert.ok(r.issues.some((i) => i.includes('N/A 但未附理由')), r.issues.join('; '));
  await rmrf(tmp);
});
...
test('Gate: du-guidance Verification 空 → failed', async () => {
  ...
  await writeFile(
    join(changeDir, 'tasks.md'),
    duSection(['- Implementation Sketch: Controller → Service', '- Pseudocode: N/A（配置变更）', '- Verification:']),
    'utf8'
  );
  const r = await runMachineGate(changeDir, gateOf('tasks.md', 'du-guidance'));
  assert.ok(r.issues.some((i) => i.includes('Verification 为空')), r.issues.join('; '));
  await rmrf(tmp);
});
...
test('Gate: du-guidance metadata 缺 guidance 段（v0.1 旧文件）→ failed；非法 trigger → failed', async () => {
  ...
  const duMetaPath = join(changeDir, 'FEAT-001', 'FEAT-001-01', 'FEAT-001-01-01', 'STORY-001-01-01-01', 'DU-BE-001', 'metadata.yaml');
  const raw = await readFile(duMetaPath, 'utf8');
  const v1 = raw
    .split('\n')
    .filter((l) => !l.includes('implementation-guidance') && !l.includes('sketch:') && !l.includes('pseudocode:') && !l.includes('complexity-trigger:'))
    .join('\n');
  await writeFile(duMetaPath, v1, 'utf8');
  await writeFile(join(changeDir, 'tasks.md'), duSection(FULL_FIELDS), 'utf8');
  let r = await runMachineGate(changeDir, gateOf('tasks.md', 'du-guidance'));
  assert.ok(r.issues.some((i) => i.includes('缺少 implementation-guidance 段')), r.issues.join('; '));
  ...
  await writeFile(duMetaPath, doc.replace('complexity-trigger:\n    - business-flow', 'complexity-trigger:\n    - bad-trigger'), 'utf8');
  r = await runMachineGate(changeDir, gateOf('tasks.md', 'du-guidance'));
  assert.ok(r.issues.some((i) => i.includes('complexity-trigger 非法值')), r.issues.join('; '));
  await rmrf(tmp);
});
...
test('Gate: du-guidance tasks.md 无 DU 小节 → failed', async () => {
  ...
  assert.ok(r.issues.some((i) => i.includes('无对应小节')), r.issues.join('; '));
  await rmrf(tmp);
});
...
test('materialize: repo metadata 复制 guidance 段；task.md 9 节 + pseudocode 提示；implementation.md 含 Deviations', async () => {
  ...
  assert.deepEqual(rm2['implementation-guidance']['complexity-trigger'], ['business-flow', 'state-transition']);

  const taskMd = await readFile(join(repoDuDir, 'task.md'), 'utf8');
  for (const sec of ['## 1. Goal', '## 2. Repository', '## 3. Scope', '## 4. Design References', '## 5. Dependencies', '## 6. Acceptance Criteria', '## 7. Implementation Sketch', '## 8. Pseudocode', '## 9. Verification']) {
    assert.ok(taskMd.includes(sec), `task.md 缺少 ${sec}`);
  }
  assert.ok(taskMd.includes('必填（complexity-trigger: business-flow, state-transition）'), taskMd);
  ...
  await writeWorkspaceDu(changeDir, await readMetadata(changeDir), { id: 'DU-BE-001', repository: 'backend', scope: ['s'], acceptance: ['a'] });
  await materializeDeliveryUnit(tmp, changeId, 'DU-BE-001');
  const taskMd2 = await readFile(join(repoDuDir, 'task.md'), 'utf8');
  assert.ok(taskMd2.includes('Pseudocode 条件必填（本 DU 未声明触发器，未命中时写 N/A + 理由）'), taskMd2);
  await rmrf(tmp);
});
