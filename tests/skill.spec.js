...
// 移除 SkillRunner 端到端测试（skill run 已移除，编排逻辑移入 SKILL.md，Agent 直接执行）
...
test('SkillLoader: 加载 sdd-feature-tree 返回正确元数据', async () => {
  const loaded = await loadSkill('sdd-feature-tree', harnessRoot);
  assert.equal(loaded.id, 'sdd-feature-tree');
  assert.equal(loaded.yaml.stage, 'feature-tree');
  assert.ok(loaded.skillMd.length > 0);
});

test('SkillLoader: 加载 sdd-knowledge 返回正确元数据', async () => {
  const loaded = await loadSkill('sdd-knowledge', harnessRoot);
  assert.equal(loaded.id, 'sdd-knowledge');
  assert.equal(loaded.yaml.stage, 'knowledge');
  assert.ok(loaded.skillMd.length > 0);
});
...
test('ArtifactWriter: 读模板 + 填 front-matter + 替换 placeholder + 写文件', async () => {
  ...
  assert.ok(raw.includes('# REQ-XXX'), 'front-matter 注释应保留');
  await rmrf(tmp);
});
