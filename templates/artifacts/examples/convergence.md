...
**项 3: 参数校验模式**
- 文件: standards/engineering/architecture-principles.md
- 操作: 合并（追加到"模块边界"段落）
...
- R1: 注册入口支持邮箱、手机号（至少提供一项）
- R2: 密码要求 ≥8 位，含大小写字母和数字
- R3: 注册成功后自动登录，无需二次验证（验证码后续迭代）
```
...
2. 分类: 3 standards + 1 spec 晋升候选 + 1 feature-tree + 2 no-update
3. 调用 sdd-knowledge 能力 A：
   - 合并 3 项到 standards/engineering/security-guidelines.md、coding-standard.md、architecture-principles.md
   - 整理 Spec 晋升候选（待人工评审后落 product/specs/账户能力.md）
...
- [x] 知识分类完成（3 standards / 1 spec 候选 / 1 feature-tree / 2 no-update）
- [x] standards 更新已写入（engineering/security-guidelines.md、engineering/coding-standard.md、engineering/architecture-principles.md）
- [x] Spec 晋升候选已按模板整理（待人工评审）
