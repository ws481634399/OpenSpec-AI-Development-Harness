...
- [ ] **Schema**：v3 metadata 能正确读写；stories 列表与目录结构一致；change split-story 命令执行后目录正确 + Gate 历史保留
- [ ] **全链路测试**：多 Story 场景（1 Change → 3 Story → 5 DU）完整跑通 lifecycle，Change.completed 仅在所有 Story.completed 时触发
- [ ] **向后兼容**：所有现有测试（401 项）不变或最小修改通过；v2 存量 Change 执行 upgrade 后正常 workflow run
- [ ] **门禁正确**：Story 级 cross-reference 缺时 Machine Gate 必 fail（blocking）；Change 级 change-prd 修改后未完成 Story 的 Gate 全部报 stale
- [ ] **轻量化不退化**：单 Story light 档场景步骤数 ≤ 当前流程（inline 模式保证）
- [ ] **CLI 冒烟**：`change split-story` / `story list/show` / `change status`（新表格输出）正常工作
